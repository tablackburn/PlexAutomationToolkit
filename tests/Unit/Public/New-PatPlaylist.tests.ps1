...
Describe 'New-PatPlaylist' {
    ...
    Context 'When creating a new playlist' {
        ...
        It 'Creates a playlist with the specified title' {
            { New-PatPlaylist -Title 'My New Playlist' -ServerUri 'http://plex.local:32400' -Confirm:$false } |
                Should -Not -Throw
        }
        ...
        It 'Returns created playlist when PassThru is specified' {
            $result = New-PatPlaylist -Title 'Test Playlist' -ServerUri 'http://plex.local:32400' -PassThru -Confirm:$false
        ...
        It 'Returns nothing when PassThru is not specified' {
            $result = New-PatPlaylist -Title 'Test Playlist' -ServerUri 'http://plex.local:32400' -Confirm:$false
    ...
    Context 'When creating playlist with specific type' {
        ...
        It 'Accepts video type' {
            { New-PatPlaylist -Title 'Video Playlist' -Type 'video' -ServerUri 'http://plex.local:32400' -Confirm:$false } |
                Should -Not -Throw
        }

        It 'Accepts audio type' {
            { New-PatPlaylist -Title 'Audio Playlist' -Type 'audio' -ServerUri 'http://plex.local:32400' -Confirm:$false } |
                Should -Not -Throw
        }

        It 'Accepts photo type' {
            { New-PatPlaylist -Title 'Photo Playlist' -Type 'photo' -ServerUri 'http://plex.local:32400' -Confirm:$false } |
                Should -Not -Throw
        }

        It 'Defaults to video type' {
            New-PatPlaylist -Title 'Default Type' -ServerUri 'http://plex.local:32400' -Confirm:$false
    ...
    Context 'When using default server' {
        ...
        It 'Uses default server when ServerUri not specified' {
            New-PatPlaylist -Title 'Default Server Test' -Confirm:$false
    ...
    Context 'When no default server is configured' {
        ...
        It 'Throws an error indicating no default server' {
            { New-PatPlaylist -Title 'No Server' -Confirm:$false } | Should -Throw '*No default server configured*'
        }
    }
    ...
    Context 'When API call fails' {
        ...
        It 'Throws an error with context' {
            { New-PatPlaylist -Title 'Fail Test' -ServerUri 'http://plex.local:32400' -Confirm:$false } |
    ...
    Context 'ShouldProcess behavior' {
        ...
        It 'Supports WhatIf' {
            New-PatPlaylist -Title 'WhatIf Test' -ServerUri 'http://plex.local:32400' -WhatIf
